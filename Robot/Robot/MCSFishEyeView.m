...
- (void)deselectSelectedItemAnimated:(BOOL)animated index:(NSInteger)deselectedIndex
{
  [self setSelectedIndex:NSNotFound withDelegateCalls:NO animated:YES];
  [self collapseAnimated:animated notifying:NO];
  [self fishEyeView:self didSelectItemAtIndex:deselectedIndex];
}

#pragma mark - delegate
- (void)fishEyeView:(MCSFishEyeView *)fishEyeView didSelectItemAtIndex:(NSUInteger)deselectedIndex{
//  0--off 1 -on
    //yes== 最开始选中， NO ==取消
    if(_Selected == NO){
        _Selected = YES;
  ...
  if (deselectedIndex==0) {
    NSLog(@"gravity");
    MCSFishEyeViewItem* fishView= [self itemAtIndex:deselectedIndex];
    [fishView colorImgManupuplate:deselectedIndex selected:2];
    [fishView defaultColor];
    [self.delegate fishEyeView:self gravityset:1];
  } else if (deselectedIndex==1){
    NSLog(@"camera Hongwai");
    [self.delegate fishEyeView:self cameraRed:0];
  } else if (deselectedIndex==2) {
    NSLog(@"speed control");
    if(_Selected == YES)
        [self.delegate fishEyeView:self speedControl:0];
    else
        [self.delegate fishEyeView:self speedControl:1];
  } else if (deselectedIndex ==3){
    NSLog(@"camera Guigan");
    if(_Selected == YES)
        [self.delegate fishEyeView:self cameraGuiGan:0];
    else
        [self.delegate fishEyeView:self cameraGuiGan:1];

  } else if (deselectedIndex==4) {
    NSLog(@"voice play");
      if(_Selected == YES){
          [self.delegate fishEyeView:self voiceplay:0];
      } else {
          [self.delegate fishEyeView:self voiceplay:1];
      }
  } else if (deselectedIndex==5) {
    NSLog(@"settting");
    [self.delegate fishEyeView:self autoSetting:0];
  } else if (deselectedIndex == 6) {
      NSLog(@"camera mode");
      if(_Selected == YES){
          [self.delegate fishEyeView:self cameraMode:0];
      } else {
          [self.delegate fishEyeView:self cameraMode:1];
      }
  }
}
...
- (void)recalculateDimensions
{
  ...
  for (int i = -self.expansionsNeighbors - 1; i < self.expansionsNeighbors + 1; i++) {
    ...
    NSLog(@"expansurplus %f,collpase %f,itemheight %f",expansionSurplus,collapsedItemHeight,itemHeight);
...
- (CGFloat)translationForOffsetFromFocusPoint:(CGFloat)offset
{
  ...
  CGFloat val = scalar*scalar*(3.0 - 2.0*scalar); // ease in out on cubic curve
    NSLog(@"tranlsationfromfocuspoint %f",val);
...
- (void)layoutItemsForOffset:(CGFloat)offset withAnimationDuration:(NSTimeInterval)duration
{
  ...
  NSLog(@"Expandedheight %f",expandedHeight);
...
//          tx = self.selectedItemOffset;
            tx =-10.0f; /// -10代表往左移 ，scale ===>代表扩大的倍数.
        }
        ty = center;
...
//        tx = [self translationForOffsetFromFocusPoint:distance];
        ...
        NSLog(@"MCF active scale %f tx %f ty %f",scale,tx,ty);
      } else {
        scale = self.collapsedItemScale;
        tx = 0.0f;
        ty = center;
...
//      NSLog(@"layoutoffset %f %f %f",layoutOffset,tx,ty);
...
//    MCSViewController* MCSV = [[MCSViewController alloc]init];
//    [MCSFishEyeView* (touch.view) deselectSelectedItemAnimated:YES];
//    [self deselectSelectedItemAnimated:YES];
    
//    for (MCSFishEyeView *fishEye in self.fishEyeViews) {
//      [fishEye deselectSelectedItemAnimated:YES];

//    [MCSV touchesBegan:touches withEvent:event];
  }
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
  ...
  NSLog(@"end");
  if ((index != NSNotFound) && (self.state!= MCSFishEyeStateExpandedPassive)) {
    [self setState:MCSFishEyeStateExpandedPassive];
//    [self setHighlightedIndex:NSNotFound];
    ...
    [self layoutItemsForOffset:offset withAnimationDuration:0.2];
      ...
      if(self.state == MCSFishEyeStateExpandedPassive){
        ...
        [self fishEyeView:self didSelectItemAtIndex:index];
      }
  }
}
